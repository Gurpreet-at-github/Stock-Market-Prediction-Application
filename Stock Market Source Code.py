...
def slice_data(df1, start_date='2019-05-18', end_date='2012-05-18', company='SPY'):
    stock_names = []
    no_of_stocks = 1
...
class Beta(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        OPTIONS = ["Select Company ", "Google", "Apple", "IBM", "Amazon", "Netflix", "Facebook","Tesla","Mutual Fund",
                   "Microsoft","Intel","Oracle","Walmart","Verizon","Bitcoin","Nike","United Health Group"]  # etc
...
class Alpha(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        date_dict = {'1 day ahead': '2019-05-29',
                     '5 days ahead': '2019-06-02',
                     '10 days ahead': '2019-06-07',
                     '20 days ahead': '2019-06-17',
                     '30 days ahead': '2019-06-27',
                     }
        def date_assign(event):
            global p_alpha
            p_alpha = str(date_dict.get(variable1.get(), "2019-05-29"))
            return
        ...
        def plot_data(X,Y, company,X_df,y_df):
            ...
            axes.plot(X_df,y_df)
            axes.plot(X,Y)
            axes.set_xlabel("Days starting from 2012-05-18 to prediction date")
...
class Portfolio(ttk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        def date_assign(event):
            global p_port_alpha
            p_port_alpha = str(date_dict.get(variable1.get(), "2019-05-29"))
            return
        w1 = ttk.OptionMenu(left_frame, variable1, *options1, command=date_assign)
        w1.pack()
        ...
        ttk.Button(left_frame, text="Microsoft", width=25, command=lambda: add_stock_to_list('MSFT')).pack()
        ttk.Button(left_frame, text="Mutual Fund", width=25, command=lambda: add_stock_to_list('AWSHX')).pack()
        ttk.Button(left_frame, text="Intel", width=25, command=lambda: add_stock_to_list('INTC')).pack()
        ttk.Button(left_frame, text="Oracle", width=25, command=lambda: add_stock_to_list('ORCL')).pack()
        ttk.Button(left_frame, text="Tesla", width=25, command=lambda: add_stock_to_list('TSLA')).pack()
        ttk.Button(left_frame, text="Verizon", width=25, command=lambda: add_stock_to_list('VZ')).pack()
        ttk.Button(left_frame, text="Bitcoin", width=25, command=lambda: add_stock_to_list('BTC-USD')).pack()
        ttk.Button(left_frame, text="Nike", width=25, command=lambda: add_stock_to_list('NKE')).pack()
        ttk.Button(left_frame, text="Walmart", width=25, command=lambda: add_stock_to_list('WMT')).pack()
        ttk.Button(left_frame, text="United Health Group", width=25, command=lambda: add_stock_to_list('UNH')).pack()

        ttk.Button(left_frame, text="Show my stocks",width=25, command=show_folio).pack(pady=2)
        ttk.Button(left_frame, text="Show portfolio vs Market", width=25, command=get_data_portfolio).pack(pady=2)
...
class PassiveTrading(ttk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        OPTIONS = ["Select Company", "SPY", "Google", "Apple", "IBM", "Amazon", "Netflix", "Facebook", "Tesla","Mutual Fund",
                    "Microsoft","Intel","Oracle","Walmart","Verizon","Bitcoin","Nike","United Health Group"]  # etc
        ...
        def plot_data(df):
            ...
            canvas._tkcanvas.pack(side=Tk.TOP, fill=Tk.BOTH, expand=1)
            label_showing=ttk.Label(bottom,text="Showing data from the given years", font=controller.general_font,justify='center')
...
class ShowStocks(ttk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        st_date_dict = {
                        '1 month data':  '2019-04-28',
                        '2 months data': '2019-03-28',
                        '3 months data': '2019-02-28',
                        '6 months data': '2018-11-28',
                       }

        def date_assign(event):
            global start_ss
            start_ss = str(st_date_dict.get(variable1.get(), "2019-04-28"))
            return
        ...
        def plot_data(df,rmean,upper,lower,company):
            ...
            axes.set_xlabel('Trading Days since Jan 2019-->')
            ...
            label_showing=ttk.Label(bottom,text="Showing data from the year 2018-2019 ", font=controller.general_font,justify='center')
        ...
        def option_selection_menu(event):
            ...
            if(start_ss=='2019-04-28'):
                window = 7
            elif(start_ss=='2019-03-28'):
                window = 15
            else:
                window = 20
...
class CompareStocks(ttk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        OPTIONS = ["Select Company to compare ","Google","Apple","IBM","Amazon","Netflix","Facebook","Tesla","Mutual Fund",
                    "Microsoft","Intel","Oracle","Walmart","Verizon","Bitcoin","Nike","United Health Group"]  # etc
...
class PredictLongTerm(ttk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        def plot_data(X, Y, company, X_df, y_df):
            ...
            axes.plot(X_df, y_df)
            axes.plot(X, Y)
            axes.set_xlabel("Days of years 2012-2019")
            axes.set_ylabel("Prices")
